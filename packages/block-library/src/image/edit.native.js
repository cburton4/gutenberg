...
class ImageEdit extends React.Component {
	...
	render() {
		...
		// We still want to render the caption so that the soft keyboard is not forced to close
		const captionDisplay = () => {
			const isCaptionEmpty = RichText.isEmpty( caption ) > 0;
			const shouldCaptionBeVisible = ! isCaptionEmpty || isSelected;
			return shouldCaptionBeVisible ? 'flex' : 'none';
		};
		...
		const getImageComponent = ( openMediaOptions, getMediaOptions ) => (
			...
			>
				<View style={ { flex: 1 } }>
					...
					/>
					<View style={ { padding: 12, flex: 1, display: captionDisplay() } }
						accessible={ true }
						accessibilityLabel={
							isEmpty( caption ) ?
